...
static char *spidev_path[MAX_BOARDS] = { "/dev/spidev2.0" };
...
typedef struct {
    ...
    __u32 trxbuf[MAX_TRX];
    __u32 *scatter[MAX_TRX];
    int nbuf;
} hm2_spi_t;
...
static __u32 read_command(__u16 addr, unsigned nelem) {
    bool increment = true;
    return (addr << 16) | 0xA000 | (increment ? 0x800 : 0) | (nelem << 4);
}

static __u32 write_command(__u16 addr, unsigned nelem) {
    bool increment = true;
...
static int do_pending(hm2_spi_t *this) {
    ...
    t = this->settings;
    t.tx_buf = t.rx_buf = (__u64)(uintptr_t)this->trxbuf;
    t.len = 4 * this->nbuf;
    ...
    __u32 *buf = this->trxbuf;
    __u32 **scatter = this->scatter;
    int i=0;
    for(i=0; i<this->nbuf; i++) {
        __u32 *target = scatter[i];
...
static int queue_write(hm2_lowlevel_io_t *llio, u32 addr, void *buffer, int size) {
    hm2_spi_t *this = (hm2_spi_t*) llio;
    if(size == 0) return 0;
...
static int queue_read(hm2_lowlevel_io_t *llio, u32 addr, void *buffer, int size) {
    hm2_spi_t *this = (hm2_spi_t*) llio;
    if(size == 0) return 0;
    if(size % 4) return -EINVAL;
...
static int do_write(hm2_lowlevel_io_t *llio, u32 addr, void *buffer, int size) {
    hm2_spi_t *this = (hm2_spi_t*) llio;
...
static int do_read(hm2_lowlevel_io_t *llio, u32 addr, void *buffer, int size) {
    hm2_spi_t *this = (hm2_spi_t*) llio;
...
static int spidev_set_lsb_first(int fd, __u8 lsb_first) {
    return ioctl(fd, SPI_IOC_WR_LSB_FIRST, &lsb_first);
}

static int spidev_set_mode(int fd, __u8 mode) {
    return ioctl(fd, SPI_IOC_WR_MODE, &mode);
}

static int spidev_set_max_speed_hz(int fd, __u32 speed_hz) {
    return ioctl(fd, SPI_IOC_WR_MAX_SPEED_HZ, &speed_hz);
}

static int spidev_set_bits_per_word(int fd, __u8 bits) {
...
static int check_cookie(hm2_spi_t *board) {
    __u32 cookie[4];
    __u32 xcookie[4] = {0x55aacafe, 0x54534f48, 0x32544f4d, 0x00000400};
